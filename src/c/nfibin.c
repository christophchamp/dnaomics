...
DESCRIPTION: Renders plots for NFI Bins
    - argv 1  Nfi Min Range
    - argv 2  Nfi Max Range
    - argv 3  Fasta file to read from
    - argv 4  Position Start Range
    - argv 5  Position Stop Range
    - argv 6  OutPut data file
    - argv 7  Numeric plot flag (0: No plot, 1: Plot)
*/
...
int main(int argc,char *argv[])
{
	...
	sprintf(table,"%s.%s_nfihunt","NFI_DNA",query);

	sprintf(filename,"%s.dat",argv[6]);
	file=fopen(filename,"w");

	sprintf(line,"(position between %s and %s) and",argv[4],argv[5]);
	sprintf(query,"select position-%s from %s where %s (nfiscore between %s and %s)",argv[4],table,line,argv[1],argv[2]);
