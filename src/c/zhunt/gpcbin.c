...
DESCRIPTION: Renders plots for GpC Content Percentage Bins
    - argv 1  Fasta file from which to read
    - argv 2  Start position range
    - argv 3  Stop position range
...
int main(int argc,char *argv[])
{
	...
	for(i=0,GpC_Content=0;i<100;i++,GpC_Content=0)
	{
		for(j=(i*(seqlength/100))+(atoi(argv[2]));j<((i+1)*(seqlength/100))+(atoi(argv[2]));j++)
			if((sequence[j]=='c'&&sequence[j+1]=='g')||(sequence[j]=='g'&&sequence[j+1]=='c'))
				GpC_Content++;
		GpC_Content=(10000*GpC_Content)/seqlength;
		fprintf(file,"%d %d\n",i,GpC_Content);
		total+=GpC_Content;
	}
