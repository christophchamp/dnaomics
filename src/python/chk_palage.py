...
for i in range(10,51):
    n = num_instances(i, False)
    if n > 0:
        print i, n

# Find the years when daughter's age is a palindrome of mother's
mum = 1950
me = 1976
for year in range(mum, mum+100):
    if ( str(year-mum)[::-1] == str(year-me) ) or \
       ( str(year-mum)[::-1] == str(year-(me+1)) ):
        print year, str(year-mum), str(year-(me+1)), str(year-me)
